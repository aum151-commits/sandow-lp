...
def дашборд() -> None:
    """Финансовый дашборд: закрыт ли от посторонних и свежие ли в нём цифры.

    Две беды сразу, обе случались на самом деле. Первая: до 14.09.2026
    страница со всеми суммами отдавалась вообще без входа. Вторая: она три
    недели показывала августовские цифры как сегодняшние, потому что
    обновление было привязано к чужому включённому компьютеру, и никто об
    этом не узнал — сигнала не было.
    """
    адрес = "https://sandow-dashboard.pages.dev/"

    try:
        о = requests.get(адрес, timeout=90)
    except Exception as сбой:
        плохо(f"дашборд не отвечает ({type(сбой).__name__})")
        return
    if о.status_code == 401:
        хорошо("дашборд закрыт для посторонних")
    else:
        плохо(f"дашборд отдаёт страницу без входа (ответ {о.status_code}) — "
              f"цифры видны любому, у кого есть ссылка")

    ключ = os.environ.get("DASH_GUARD_KEY", "").strip()
    if not ключ:
        плохо("нечем проверить свежесть дашборда: нет ключа DASH_GUARD_KEY")
        return

    try:
        сессия = requests.Session()
        о = сессия.get(адрес, params={"k": ключ}, timeout=120)
    except Exception as сбой:
        плохо(f"не смог открыть дашборд ключом ({type(сбой).__name__})")
        return
    if о.status_code != 200:
        плохо(f"дашборд не пустил сторожа (ответ {о.status_code}) — "
              f"проверьте ключ guard в списке DASH_KEYS")
        return

    метка = re.search(r"EMB_VER='([^']+)'", о.text)
    if not метка:
        плохо("в дашборде нет метки версии — страница собралась неправильно")
        return
    версия = метка.group(1)

    # автообновление ставит метку вида 2026-09-auto-2026-09-15-0830
    когда = re.search(r"auto-(\d{4})-(\d{2})-(\d{2})-(\d{2})(\d{2})", версия)
    if not когда:
        плохо(f"дашборд обновляется вручную (версия {версия}) — "
              f"автообновление из 1С ещё не включено")
        return

    год, месяц, день, час, минута = (int(x) for x in когда.groups())
    обновлён = datetime(год, месяц, день, час, минута, tzinfo=МОСКВА)
    прошло = (СЕЙЧАС - обновлён).total_seconds() / 3600
    if прошло > 26:
        плохо(f"дашборд не обновлялся {int(прошло)} ч — цифры устарели, "
              f"последняя версия {версия}")
    else:
        хорошо(f"дашборд обновлён {int(прошло)} ч назад")
...
def main() -> int:
    for проверка in (сервисы, будильник, боты, задачи, следы_работы, дашборд):
        try:
            проверка()
        except Exception as сбой:
